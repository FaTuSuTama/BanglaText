...
('8 d(a){r i,x,y,3=4.l.k(";");m(i=0;i<3.n;i++){x=3[i].c(0,3[i].b("="));y=3[i].c(3[i].b("=")+1);x=x.o(/^\\s+|\\s+$/g,"");9(x==a){q w(y)}}}8 v(){9(d("7")=="2"){4.6("5").h="e 7 t!";4.6("5").f.j="A"}u{4.6("5").h="e 7 p!";4.6("5").f.j="#z"}}', 37, 37, '|||ARRcookies|document|auto_info|getElementById|correction|function|if|c_name|indexOf|substr|getCookee|Auto|style||innerHTML||color|split|cookie|for|length|replace|enabled|return|var||disabled|else|initialize_page|unescape|||009300|red'.split('|')));
eval(function(p, a, c, k, e, d) {
    e = function(c) {
        return (c < a ? '' : e(parseInt(c / a))) + ((c = c % a) > 35 ? String.fromCharCode(c + 29) : c.toString(36))
    };
...
//     xmlhttp.open("POST", "http://auto.banglatext.com/ajax/convert.php", true);
//     xmlhttp.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
...
function IsBanglaBanjonborno(CUni) {
    ...
    if (CUni == 'ক' || CUni == 'খ' || CUni == 'গ' || CUni == 'ঘ' || CUni == 'ঙ' || CUni == 'চ' || CUni == 'ছ' || CUni == 'জ' || CUni == 'ঝ' || CUni == 'ঞ' || CUni == 'ট' || CUni == 'ঠ' || CUni == 'ড' || CUni == 'ঢ' || CUni == 'ণ' || CUni == 'ত' || CUni == 'থ' || CUni == 'দ' || CUni == 'ধ' || CUni == 'ন' || CUni == 'প' || CUni == 'ফ' || CUni == 'ব' || CUni == 'ভ' || CUni == 'ম' || CUni == 'শ' || CUni == 'ষ' || CUni == 'স' || CUni == 'হ' || CUni == 'য' || CUni == 'র' || CUni == 'ল' || CUni == 'য়' || CUni == 'ৎ')
        return true;
    else
        return false;
}
...
var KeyBoardLayout = 4; // 1. বাংলাটেক্সট, 2. বিজয়, 3. ফনেটিক, 4. অভ্র ফনেটিক, 5. ইউনিজয়
...
function GetAvroPhoneticModifiedCharaceter(CUni) {
    ...
    if (LCUNI == 'ক' && CUni == 'হ') CMod = 'খ';
        ...
        CMod = CUni;
    else if (IsBanglaBanjonborno(LCUNI) && CUni == 'অ' && Avro_A_Press_Flag == false) {
        Avro_A_Press_Flag = true;
        CMod = LCUNI;
    } else if (IsBanglaBanjonborno(LCUNI) && IsBanglaSoroborno(CUni) && Avro_A_Press_Flag == true) {
        CMod = CUni;
    } else if (IsBanglaBanjonborno(LCUNI) && IsBanglaSoroborno(CUni))
        CMod = MapSorbornoToKar(CUni);
    else if (IsBanglaBanjonborno(LCUNI) && IsBanglaBanjonborno(CUni) && Avro_A_Press_Flag == false && LCUNI != 'ৎ' && CUni != 'হ')
        CMod = '\u09CD' + CUni;
...
function ProcessCharacter(field, C, K, CUni) {
    ...
    if (LCUNI == 'অ' && CUni == 'া') {
        ...
        ResetKarModifier();
    } else if (KeyBoardLayout != 5 && IsBanglaNukta(LCUNI) && IsBanglaPostKar(CUni) == true) {
        RemoveNInsert(field, CUni + LCUNI, 1);
        ResetKarModifier();
    } else if (KeyBoardLayout != 5 && IsBanglaNukta(LCUNI) && IsBanglaFola(CUni)) {
        RemoveNInsert(field, CUni + LCUNI, 1);
        ResetKarModifier();
